...
interface FAQNotesEditorProps {
  faqIndex: number;
  initialNotes?: string;
}

export function FAQNotesEditor({ faqIndex, initialNotes = "" }: FAQNotesEditorProps) {
  const [notes, setNotes] = useState(initialNotes);
  const [isEditing, setIsEditing] = useState(false);
  const storageKey = `faq-notes-${faqIndex}`;
  ...
  const handleCancel = () => {
    const savedNotes = localStorage.getItem(storageKey) || initialNotes;
    setNotes(savedNotes);
    setIsEditing(false);
  };
  ...
  return (
    <div className="mt-4 p-4 bg-gray-50 border-l-4 border-gray-300 rounded">
      ...
      >
        {notes || "Click here to add your notes..."}
      </button>
    </div>
  );
}
